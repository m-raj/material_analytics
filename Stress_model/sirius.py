...
class Dataset(Sequence):
    ...
    def __len__(self):
        return math.ceil(len(self.phase_dist) / self.batch_size)

    def __getitem__(self, index):
        element_ids = self.phase_dist.index[index *
...
# plot_model(model,to_file='model_plot.png',show_shapes=True,expand_nested=True)


dataset = Dataset(frame_description,
...
logs = model.fit(dataset, epochs=EPOCHS, workers=NUM_WORKERS, use_multiprocessing=True)
...
# Prediction
result = model.predict(dataset, workers=NUM_WORKERS, use_multiprocessing=True)
np.save('predicted_stress', result)
# Save model
model.save('model')
